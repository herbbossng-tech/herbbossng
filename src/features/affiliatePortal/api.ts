...
export async function fetchMyDashboard(): Promise<AffiliateDashboard> {
  const { data, error } = await supabaseAffiliate.rpc('get_my_affiliate_dashboard').single()
...
export async function fetchCampaignProducts(campaignId: string) {
  ...
  const { data, error } = await supabaseAffiliate.from('products').select('id, name, selling_price').in('id', productIds)
  if (error) throw error
  return data ?? []
}

export async function fetchMyOrderForms(): Promise<AffiliateOrderForm[]> {
  const { data, error } = await supabaseAffiliate.from('affiliate_order_forms').select('*').is('deleted_at', null).order('created_at', { ascending: false })
  if (error) throw error
