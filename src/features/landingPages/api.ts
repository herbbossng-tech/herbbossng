...
import type { LandingPageFilters, LandingPageListItem, PaginatedLandingPages } from './types'
...
/**
 * Creates a minimal draft row, then seeds the template's starter
 * section list — the same "create minimal row, then edit" flow the
 * Products module already uses. A brand-new page is never published;
 * status defaults to 'draft' at the table level.
 */
