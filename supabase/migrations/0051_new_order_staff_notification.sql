-- ============================================================
-- GOLDEN COMMERCE OS — "New Order Received" staff/admin email
-- notification (the 'new_order' key the original email_templates
-- table comment (0006) always listed but no migration ever seeded).
--
-- This is deliberately NOT a new hardcoded email — it reuses the
-- existing communication-template + automation infrastructure
-- (0028/0034) exactly as designed:
--   1. A system-default 'new_order' email template is seeded below.
--      Like every other system template it is fully editable — a
--      workspace (or brand) creates its own template with the same
--      key via the existing Communication Templates page, which wins
--      over this system default by the standard brand > workspace >
--      system precedence (render_communication_template()). Nothing
--      about the notification's content is fixed in code.
--   2. Recipients are NOT hardcoded either. A workspace wires this up
--      itself under Automation Rules: WHEN "Order created" -> THEN
--      "Send Email", template = "New Order Received (Staff Alert)",
--      recipient = a literal staff/admin email address. Add one more
--      SEND_EMAIL action (same rule) per additional address to notify
--      more than one inbox — the rule builder already supports
--      multiple actions per rule.
--   3. execute_automation_action()'s SEND_EMAIL variable set is
--      extended (additively — the five pre-existing keys are left
--      byte-for-byte unchanged) so a template can reference the
--      customer's phone/email/address/city/state, what was ordered,
--      amounts, the order date, and a human status label.
--   4. create_order()/create_public_order() are extended to stash a
--      plain-text item summary onto orders.metadata at INSERT time
--      (item_summary/item_quantity). This is required, not
--      cosmetic: emit_automation_event() is fired by an AFTER INSERT
--      trigger on orders and runs process_automation_event()
--      synchronously in the SAME transaction, which is BEFORE the
--      calling function goes on to insert order_items — so an
--      order-created automation action can never see order_items via
--      a live join. Stashing the summary in the orders row itself at
--      insert time (both functions already know the item(s) before
--      that insert) sidesteps the race entirely.
-- ============================================================


-- ----------------------------------------------------------------
-- PART A — create_order(): stash an item summary in metadata.
-- This is the 0024 signature/body (the current one — 0024 added
-- p_affiliate_referral_code and rewrote customer matching/affiliate
-- attribution after 0021), unchanged except for the item_summary/
-- item_quantity accumulation and the new metadata column in the
-- orders insert. Every other line — canonical-phone customer
-- matching, affiliate referral resolution, campaign/commission
-- processing — is byte-for-byte identical to 0024.
-- ----------------------------------------------------------------
...
begin
  ...
  insert into public.orders (
    ...
    jsonb_build_object('item_summary', array_to_string(v_item_summary, ', '), 'item_quantity', v_item_quantity),
    auth.uid(), auth.uid()
  )
...
comment on function public.create_order(
  ...
  'The only way to create an order. Prices/costs are always read from the live products row server-side. Finds-or-creates the ordering customer by canonical phone and links order.customer_id. Snapshots order_items.unit_cost for Product Performance COGS attribution. Resolves an optional affiliate referral code and, once order_items exist, the best-matching ACTIVE campaign — firing the commission engine immediately for PER_ORDER_CREATED campaigns. metadata.item_summary/item_quantity (0051) are computed from p_items before the orders insert so an orders.created automation action can read what was ordered without racing order_items (see 0051''s header comment).';


-- ----------------------------------------------------------------
-- PART B — create_public_order(): same idea, one package per order.
-- This is the 0042 signature/body (the current one — 0031/0041/0042
-- each extended it after 0021: market-aware currency/phone, required
-- field enforcement, canonical-phone customer matching, affiliate
-- attribution, UTM/fbclid/ttclid columns, immediate ORDER_CREATED +
-- PURCHASE tracking events), unchanged except the two new keys added
-- to the existing orders.metadata jsonb_build_object. Every other
-- line is byte-for-byte identical to 0042.
-- ----------------------------------------------------------------
...
begin
  ...
  begin
    case v_action_type
      ...
      -- extended again in 0051 (additive variables only):
...
'<div style="background:#0b0e0c;color:#e5e7eb;font-family:Arial,Helvetica,sans-serif;max-width:600px;margin:0 auto;border-radius:8px;overflow:hidden;">
  ...
  <div style="background:#141614;padding:14px 24px;color:#6b7280;font-size:12px;">Thank you for choosing us. &middot; Sent via {{brand_name}}</div>
    ...
    '["order_number","customer_name","customer_phone","customer_email","customer_address","customer_city","customer_state","package_name","quantity","currency","subtotal","shipping_fee","order_total_display","order_date","order_status","brand_name"]'::jsonb)
